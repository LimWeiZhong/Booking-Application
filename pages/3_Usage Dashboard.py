...
with st.container():
    ...
    met2.metric(
        ...
        value=filtered_transactions.shape[0] if not filtered_transactions.empty else 0,
        help="Total number of bookings for the selected period"
    )
    met3.metric(
        label="Unique Users",
        value=filtered_transactions['User'].nunique() if not filtered_transactions.empty else 0,
